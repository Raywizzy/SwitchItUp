...
11. SwitchAI generates an outfit, explains the choice, suggests replacements, and learns from feedback.

## Frontend MVP Scope

- Responsive app shell with dark navigation rail, setup progress, wardrobe rail, virtual styling canvas, SwitchAI panel, stylist request form, stylist marketplace, mall wishlist, social feed, and competition panel.
- Backend-aware UI for role switching, wardrobe filtering, adding wardrobe photos, applying clothes to the virtual body, marking 360 scan captured, running SwitchAI, sending feedback, sending stylist requests, and accepting/discarding wishlist items.
...
- `src/backend.py` stores app data in per-session `data/app_state-*.json` files locally or per-session Supabase/Postgres JSONB rows when hosted persistence credentials are configured. It exposes service methods for profile role, measurements, wardrobe, selected outfit, scan capture, SwitchAI recommendations and feedback, style requests, wishlist decisions, stylist upgrades, social posts/reactions, direct messages, mall registration, competitions, competition entries, and stylist follows.
- `src/switch_ai.py` scores wardrobe items, selects category-complete outfits, recommends stylists, proposes mall wishlist replacements, explains decisions, and updates session preferences from feedback.
- The backend also reuses the rule-based style engine to create real outfit plans from the saved wardrobe.
...
- Model-backed vision and embeddings for SwitchAI
